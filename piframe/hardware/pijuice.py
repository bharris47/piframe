...
pijuice_user_functions = ["USER_EVENT"] + [
...
class PiJuiceInterface(object):
    ...
    def _DoTransfer(self, oper):
        if (self.t != None and self.t.is_alive()) or (
...
class PiJuiceStatus(object):
    ...
    def GetFaultStatus(self):
        ...
        if result["error"] != "NO_ERROR":
            ...
            if (d >> 6) & 0x03:
                fault["charging_temperature_fault"] = batChargingTempEnum[
                    (d >> 6) & 0x03
                ]
    ...
    def GetAlarm(self):
        ...
        if (d[2] & 0x80) == 0x00:
            if d[2] & 0x40:
                ...
                ampm = "PM" if (d[2] & 0x20) else "AM"
                alarm["hour"] = (
                    str(((d[2] >> 4) & 0x01) * 10 + (d[2] & 0x0F)) + " " + ampm
                )
            else:
                # hourFormat = '24'
...
class PiJuicePower(object):
    ...
    def SetWatchdog(self, minutes, non_volatile=False):
        try:
            ...
            if d >= 0x4000:
                d = (d >> 2) | 0x4000  # correct resolution
            d = d | nv
        except:
            return {"error": "BAD_ARGUMENT"}
        return self.interface.WriteData(
            self.WATCHDOG_ACTIVATION_CMD, [d & 0xFF, (d >> 8) & 0xFF]
        )
    ...
    def GetWatchdog(self):
        ...
        minutes = minutes << (
            (cfg & 0x4000) >> 13
        )  # correct resolution for range 16384-65536
        return {
            "data": minutes,
            "non_volatile": bool(cfg & 0x8000),
            "error": "NO_ERROR",
        }
...
class PiJuiceConfig(object):
    ...
    def GetChargingConfig(self):
        ...
        if ret["error"] != "NO_ERROR":
            ...
            return {
                "data": {"charging_enabled": bool(ret["data"][0] & 0x01)},
    ...
    def GetBatteryProfileStatus(self):
        ...
        if ret["error"] != "NO_ERROR":
            ...
            if id == 0xF0:
                return {
                    "data": {"validity": "DATA_WRITE_NOT_COMPLETED"},
                    "error": "NO_ERROR",
                }
    ...
    def GetBatteryProfile(self):
        ...
        if ret["error"] != "NO_ERROR":
            ...
            profile["capacity"] = (
                0xFFFFFFFF
                if (packed_u16 == 0xFFFF)
                else (packed_u16 & 0x7FFF) << (((packed_u16 & 0x8000) >> 15) * 7)
            )
    ...
    def SetCustomBatteryProfile(self, profile):
        ...
        try:
            ...
            if cap == 0xFFFFFFFF:
                ...
                packed_u16 = (c >> (int(c >= 0x8000) * 7)) | int(
                    c >= 0x8000
                ) * 0x8000  # correction for large capacities over 32767
            d[0] = packed_u16 & 0xFF
    ...
    def GetBatteryTempSenseConfig(self):
        ...
        if result["error"] != "NO_ERROR":
            ...
            if (d[0] & 0x07) < len(self.batteryTempSenseOptions):
                return {
                    "data": self.batteryTempSenseOptions[d[0] & 0x07],
                    "error": "NO_ERROR",
                }
            else:
    ...
    def GetRsocEstimationConfig(self):
        ...
        if result["error"] != "NO_ERROR":
            ...
            if ((d[0] & 0x30) >> 4) < len(self.rsocEstimationOptions):
                return {
                    "data": self.rsocEstimationOptions[(d[0] & 0x30) >> 4],
                    "error": "NO_ERROR",
                }
            else:
                return {"error": "UNKNOWN_DATA"}
            ...
            for i in range(0, len(self.buttonEvents)):
                ...
                try:
                    if d[i * 2] == 0:
                        ...
                        config[self.buttonEvents[i]]["function"] = (
                            pijuice_hard_functions[(d[i * 2] & 0x0F) - 1]
                        )
                    elif d[i * 2] & 0xF0 == 0x10:
                        config[self.buttonEvents[i]]["function"] = (
                            pijuice_sys_functions[(d[i * 2] & 0x0F) - 1]
                        )
                    elif d[i * 2] & 0xF0 == 0x20:
                        config[self.buttonEvents[i]]["function"] = (
                            pijuice_user_functions[d[i * 2] & 0x0F]
                        )
                    else:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        data[i * 2] = 0
            data[i * 2 + 1] = (
                int(config[self.buttonEvents[i]]["parameter"]) // 100
            ) & 0xFF
        return self.interface.WriteDataVerify(
            self.BUTTON_CONFIGURATION_CMD + b, data, 0.4
        )
    ...
    ioModes = [
        ...
        "ANALOG_IN",
        "DIGITAL_IN",
        "DIGITAL_OUT_PUSHPULL",
        "DIGITAL_IO_OPEN_DRAIN",
        "PWM_OUT_PUSHPULL",
        "PWM_OUT_OPEN_DRAIN",
    ]
    ioSupportedModes = {
    ...
    def SetIoConfiguration(self, io_pin, config, non_volatile=False):
        ...
        try:
            ...
            if config["mode"] == "DIGITAL_IN":
                ...
                config["mode"] == "DIGITAL_OUT_PUSHPULL"
                or config["mode"] == "DIGITAL_IO_OPEN_DRAIN"
            ):
                d[1] = int(config["value"]) & 0x01  # output value
            elif (
                config["mode"] == "PWM_OUT_PUSHPULL"
                or config["mode"] == "PWM_OUT_OPEN_DRAIN"
            ):
                p = int(config["period"])
                if p >= 2:
                    p = p // 2 - 1
            ...
            return {"error": "INVALID_CONFIG"}
        return self.interface.WriteDataVerify(
            self.IO_CONFIGURATION_CMD + (io_pin - 1) * 5, d, 0.2
        )
    ...
    def GetIoConfiguration(self, io_pin):
        ...
        if ret["error"] != "NO_ERROR":
            ...
            nv = bool(d[0] & 0x80)
            mode = (
                self.ioModes[d[0] & 0x0F]
                if ((d[0] & 0x0F) < len(self.ioModes))
                else "UNKNOWN"
            )
            pull = (
                self.ioPullOptions[(d[0] >> 4) & 0x03]
                if (((d[0] >> 4) & 0x03) < len(self.ioPullOptions))
                else "UNKNOWN"
            )
            if mode == "DIGITAL_OUT_PUSHPULL" or mode == "DIGITAL_IO_OPEN_DRAIN":
                return {
                    "data": {"mode": mode, "pull": pull, "value": int(d[1])},
                    "non_volatile": nv,
                    "error": "NO_ERROR",
                }
                ...
                wup = (
                    self.ioConfigParams["DIGITAL_IN"][0]["options"][d[1] & 0x03]
                    if d[1] & 0x03
                    < len(self.ioConfigParams["DIGITAL_IN"][0]["options"])
                    else ""
                )
                return {
                    "data": {"mode": mode, "pull": pull, "wakeup": wup},
                    "non_volatile": nv,
                    "error": "NO_ERROR",
                }
    ...
    def SetIdEepromWriteProtect(self, status):
        ...
        if status == True:
            ...
            return {"error": "BAD_ARGUMENT"}
        return self.interface.WriteDataVerify(
            self.ID_EEPROM_WRITE_PROTECT_CTRL_CMD, [d]
        )
    ...
    def SetDefaultConfiguration(self):
        return self.interface.WriteData(
            self.RESET_TO_DEFAULT_CMD, [0xAA, 0x55, 0x0A, 0xA3]
        )
